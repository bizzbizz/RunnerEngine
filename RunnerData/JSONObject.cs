		...
		void Parse(string str, int maxDepth, bool storeExcessLevels, bool strict)
		{
			if (!string.IsNullOrEmpty(str))
			{
				...
				if (strict)
				{
					...
					{
						type = Type.NULL;
						DebugError.LogWarning("Improper (strict) JSON formatting.  First character must be [ or {");
						return;
					}
				}
				if (str.Length > 0)
					...
					else if (str[0] == '"')
					{
						...
						switch (str[offset])
						{
							...
							default:
								...
								{
									type = Type.NULL;
									DebugError.LogWarning("improper JSON formatting:" + str);
								}
								return;
						}
						string propName = "";
	...
	public void GetField(ref double field, string name, FieldNotFound fail) {
		...
		static void MergeRecur(JSONObject left, JSONObject right)
		{
			...
			else if (left.type == Type.ARRAY && right.type == Type.ARRAY)
			{
				...
				{
					DebugError.LogError("Cannot merge arrays when right object has more elements");
					return;
				}
				for (int i = 0; i < right.list.Count; i++)
				{
...
#pragma warning restore 219
		...
		{   //Convert the JSONObject into a string
			...
			if (depth++ > MAX_DEPTH)
			{
				DebugError.Log("reached max depth!");
				yield break;
			}
			...
			if (type == Type.OBJECT)
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					switch (val.type)
					{
						...
						default: DebugError.LogWarning("Omitting object: " + keys[i] + " in dictionary conversion"); break;
					}
				}
				return result;
			}
			DebugError.LogWarning("Tried to turn non-Object JSONObject into a dictionary");
			return null;
		}
